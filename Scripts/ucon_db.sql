...
CREATE TABLE IF NOT EXISTS `ucon`.`educational_institutions` (
  ...
  `updated_at` DATETIME NOT NULL DEFAULT now(),
  PRIMARY KEY (`id`),
  UNIQUE INDEX `educational_institution_UNIQUE` (`educational_institution` ASC) VISIBLE)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `ucon`.`degrees` (
  ...
  `created_at` DATETIME NOT NULL DEFAULT now(),
  `updated_at` DATETIME NOT NULL DEFAULT now(),
  PRIMARY KEY (`id`, `courses_id`, `educational_institutions_id`, `teachers_users_id`),
  UNIQUE INDEX `degree_UNIQUE` (`degree` ASC) VISIBLE,
  INDEX `fk_degrees_courses1_idx` (`courses_id` ASC) VISIBLE,
  INDEX `fk_degrees_educational_institutions1_idx` (`educational_institutions_id` ASC) VISIBLE,
  INDEX `fk_degrees_teachers1_idx` (`teachers_users_id` ASC) VISIBLE,
  CONSTRAINT `fk_degrees_courses1`
    FOREIGN KEY (`courses_id`)
    REFERENCES `ucon`.`courses` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_degrees_educational_institutions1`
    FOREIGN KEY (`educational_institutions_id`)
    REFERENCES `ucon`.`educational_institutions` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_degrees_teachers1`
